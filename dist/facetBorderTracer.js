...
class FacetBorderTracer {
    ...
    static buildFacetBorderPaths(facetResult_1) {
        return __awaiter(this, arguments, void 0, function* (facetResult, onUpdate = null) {
            let count = 0;
            const borderMask = new typedarrays_1.BooleanArray2D(facetResult.width, facetResult.height);
            ...
            for (let fidx = 0; fidx < facetProcessingOrder.length; fidx++) {
                ...
                if (f != null) {
                    ...
                    const xWall = new typedarrays_1.BooleanArray2D(facetResult.width + 1, facetResult.height + 1);
                    const yWall = new typedarrays_1.BooleanArray2D(facetResult.width + 1, facetResult.height + 1);
                    ...
                    const pt = new facetmanagement_1.PathPoint(f.borderPoints[borderStartIndex], facetmanagement_1.OrientationEnum.Left);
                    // L T R B
        ...
        while (!finished) {
            ...
            if (pt.orientation === facetmanagement_1.OrientationEnum.Left) {
                ...
                if (((pt.y - 1 >= 0 && facetResult.facetMap.get(pt.x, pt.y - 1) !== f.id) // top exists and is a neighbour facet
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x, pt.y), facetmanagement_1.OrientationEnum.Top);
                ...
                if (((pt.y + 1 < facetResult.height && facetResult.facetMap.get(pt.x, pt.y + 1) !== f.id) // bottom exists and is a neighbour facet
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x, pt.y), facetmanagement_1.OrientationEnum.Bottom);
                    possibleNextPoints.push(nextpt);
                }
                // check upwards
                //   +---n---+
                ...
                if (pt.y - 1 >= 0 // top exists
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x, pt.y - 1), facetmanagement_1.OrientationEnum.Left);
                    possibleNextPoints.push(nextpt);
                }
                // check downwards
                //   +---x---+
                ...
                if (pt.y - 1 >= 0 && pt.x - 1 >= 0 // there is a left upwards
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x - 1, pt.y - 1), facetmanagement_1.OrientationEnum.Bottom);
                    possibleNextPoints.push(nextpt);
                }
                ...
                if (pt.y + 1 < facetResult.height && pt.x - 1 >= 0 // there is a left downwards
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x - 1, pt.y + 1), facetmanagement_1.OrientationEnum.Top);
                    possibleNextPoints.push(nextpt);
                }
            }
                ...
                if (((pt.x - 1 >= 0
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x, pt.y), facetmanagement_1.OrientationEnum.Left);
                    possibleNextPoints.push(nextpt);
                }
                ...
                if (((pt.x + 1 < facetResult.width
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x, pt.y), facetmanagement_1.OrientationEnum.Right);
                    possibleNextPoints.push(nextpt);
                }
                // check leftwards
                if (pt.x - 1 >= 0
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x - 1, pt.y), facetmanagement_1.OrientationEnum.Top);
                    possibleNextPoints.push(nextpt);
                }
                // check rightwards
                if (pt.x + 1 < facetResult.width
                ...
                if (pt.y - 1 >= 0 && pt.x - 1 >= 0 // there is a left upwards
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x - 1, pt.y - 1), facetmanagement_1.OrientationEnum.Right);
                    possibleNextPoints.push(nextpt);
                }
                // check right upwards
                if (pt.y - 1 >= 0 && pt.x + 1 < facetResult.width // there is a right upwards
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x + 1, pt.y - 1), facetmanagement_1.OrientationEnum.Left);
                    possibleNextPoints.push(nextpt);
                }
            }
                ...
                if (pt.y - 1 >= 0
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x, pt.y - 1), facetmanagement_1.OrientationEnum.Right);
                    possibleNextPoints.push(nextpt);
                }
                // check downwards
                if (pt.y + 1 < facetResult.height
                ...
                if (pt.x - 1 >= 0
                    ...
                    const nextpt = new facetmanagement_1.PathPoint(new point_1.Point(pt.x - 1, pt.y), facetmanagement_1.OrientationEnum.Bottom);
                    possibleNextPoints.push(nextpt);
                }
                // check rightwards
                if (pt.x + 1 < facetResult.width
